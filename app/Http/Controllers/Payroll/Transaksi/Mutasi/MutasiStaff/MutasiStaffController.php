...
namespace App\Http\Controllers\Payroll\Transaksi\Mutasi\MutasiStaff;
...
class MutasiStaffController extends Controller
{
    //Display a listing of the resource.
    public function index()
    {
        $data = 'HAPPY HAPPY HAPPY';
        return view('Payroll.Transaksi.Mutasi.Staff.mutasiStaff', compact('data'));
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::connection('ConnPayroll')->statement('exec SP_1486_PAY_INS_MUTASI_STAFF @prioritas = ?, @kdman = ?, @old_kd_pegawai = ?, @new_kd_pegawai = ?, @old_jabatan = ?, @new_jabatan = ?, @old_nm_divisi= ?, @new_nm_divisi = ?, @tgl_mutasi = ?, @no_surat = ?, @Alasan = ?', [
            $data['prioritas'],
            $data['kdman'],
            $data['old_kd_pegawai'],
            $data['new_kd_pegawai'],
